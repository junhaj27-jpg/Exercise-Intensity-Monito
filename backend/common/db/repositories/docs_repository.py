...
def get_docs_detail_by_docs_sn(prj_sn: int, docs_sn: int) -> dict[str, Any]:
    ...
    with db_connection() as conn:
        with conn.cursor() as cursor:
            ...
            row = cursor.fetchone()
    if not row:
        raise FileNotFoundError(f"Document detail not found. prj_sn={prj_sn}, docs_sn={docs_sn}")
    return row
    ...
    with db_connection() as conn:
        try:
            with conn.cursor() as cursor:
                ...
                docs_sn = int(cursor.lastrowid)
                cursor.execute(detail_sql, (docs_sn, docs_path, login_user_sn))
                docs_dtl_sn = int(cursor.lastrowid)
            conn.commit()
        except Exception:
            conn.rollback()
            raise
    return {"docs_sn": docs_sn, "docs_dtl_sn": docs_dtl_sn}
